...
while end < 10:
        ...
        #a = driver.find_elements_by_css_selector('div.v1Nh3 kIKUG  _bz0w > a')
...
for i in a_list:
    ...
    div = soup.find("div", {"class": "C4VMK"})
    if div != None:
        tags = div.findAll("a")
        tag_texts = []
        for j in tags:
            tag_texts.append(j.text)
        if '#서촌찻집' in tag_texts:
            for j in tag_texts:
                if j[0] != '#':
                    continue
                else:
                    if not (j in tags_data):
                        tags_data[j] = 0
                    tags_data[j] += 1
...
#workbook = xlsxwriter.Workbook('서촌찻집2.xlsx')
